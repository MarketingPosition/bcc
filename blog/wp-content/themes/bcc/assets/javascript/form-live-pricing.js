jQuery(document).ready(function () {
  ...
  (lead_form = jQuery(".step-lead-form")),
    ...
    (posturl = "<?php echo $domain;?>/api/form-split-test.php");
  ...
  check_postcode.on("click", function (e) {
    ...
    if (lead_form.valid() === true) {
      ...
      var wasteTypes = $('input[name="question1"]:checked')
        .map(function () {
          return this.value;
        })
        .get();
      var bags = $('input[name="question2"]:checked').val();
      var postcode = $("#postcodefield").val();

      if (wasteTypes.length === 1) {
        $.ajax({
          url: "<?php echo $domain;?>/api/waste-instant-price.php",
          dataType: "json",
          data: {
            waste: wasteTypes[0],
            bags: bags,
          ...
          success: function (response) {
            ...
            var priceFrom = response.pricesFrom;

            if (priceFrom) {
              $("#instant-price-field").val(priceFrom);
              $("#quote-container").val(container);
              $("#quote-frequency").val(frequency);
            } else {
              $("#instant-price-field").val("N/A");
            }
          },
        });
      }
  ...
  function postForm() {
    ...
    var jqxhr = jQuery.post(posturl, lead_data, function (data) {
      ...
      window.location.href =
        "<?php echo $domain;?>/thank-you/?instant-price=" +
        $("#instant-price-field").val() +
        "&container=" +
        $("#quote-container").val() +
        "&frequency=" +
        $("#quote-frequency").val();
