...
?>
      ...
      <div class="container">
        <div class="row">
          ...
          <div class="col-xs-12 col-sm-6 col-lg-offset-1 col-lg-3">
            <h4>Contact Us</h4>
            <p><a href="https://goo.gl/maps/Mb7Us1avf2te8UTy6" target="_blank">Business Cost Comparison<br>
              Floor 2, Northern Design Centre <br>
              Abbott's Hill, Gateshead <br>
              NE8 3DF</a></p>
			  
            <p>Phone: <a href="tel:03300589414">03300 589 414</a> <br>
    ...
    <?php if ( is_page_template( 'page-quote.php' ) || is_single() ) :?>
      ...
      <script type="text/javascript" src="<?php echo get_template_directory_uri(); ?>/assets/javascript/form-live-pricing.js"></script>

      <?php /* <script type="text/javascript" src="<?php echo get_template_directory_uri(); ?>/assets/javascript/form.js"></script> */ ?>
...
<?php // Loading the script only on specific pages not to have a conflict with other Js   ?>
<?php if( $post->ID == 1624) { ?>
<script>
	jQuery(document).on('gform_post_render', function(event, form_id, current_page){
	
	jQuery('#choice_6_1_7').on('change', function() {
	jQuery('#input_6_1 > li > input').not(this).prop('checked', false);
	});
	
	jQuery('#input_6_1 > li > input').on('change', function() {
	jQuery('#choice_6_1_7').not(this).prop('checked', false);
	});   
	
	var radio = jQuery('.gfield_radio input[type="radio"').val();
	if (radio == "No"){
		alert(raio);
	}
...
jQuery(document).on('gform_post_render', function(event, form_id, current_page){
	jQuery(document).ready(function(){
		jQuery('.multistep .gfield_select').addClass('select-css');
	});
});
</script>
<?php } ?>
...
<?php if( $post->ID == 1627) { ?>
<script src="/wp-content/themes/bcc/assets/javascript/custom-mkt.js"></script>
<?php } ?>
