...
if(strrpos($_SERVER['HTTP_USER_AGENT'], 'MSIE') !== false)
 ...
 elseif(strrpos($_SERVER['HTTP_USER_AGENT'], 'Trident') !== false)
    return 'Internet explorer';
 elseif(strrpos($_SERVER['HTTP_USER_AGENT'], 'Firefox') !== false)
   return 'Mozilla Firefox';
 elseif(strrpos($_SERVER['HTTP_USER_AGENT'], 'Opera Mini') !== false)
   return "Opera Mini";
 elseif(strrpos($_SERVER['HTTP_USER_AGENT'], 'Opera') !== false)
   return "Opera";
 elseif(strrpos($_SERVER['HTTP_USER_AGENT'], 'Safari') !== false)
   return "Safari";
	elseif(strrpos($_SERVER['HTTP_USER_AGENT'], 'Edge') !== false)
   return "Edge";
	elseif(strrpos($_SERVER['HTTP_USER_AGENT'], 'Chrome') !== false)
   return 'Google Chrome';
 else
   return 'Other - '.$_SERVER['HTTP_USER_AGENT'];
}
*/
...
$mkt_question1      = $_POST["mkt_question1"];

$mkt_first_name     = $_POST["first_name"];
$mkt_last_name      = $_POST["last_name"];
$mkt_company        = $_POST["company"];
$mkt_phone          = $_POST["phone"];
$mkt_email          = $_POST["email"];
$mkt_website           = $_POST["website"];



$bcchookzap = "mkt_question1=" . $mkt_question1 .
  "&mkt_first_name=" . $mkt_first_name .
  "&mkt_last_name=" . $mkt_last_name .
  "&mkt_company=" . $mkt_company .
  "&mkt_phone=" . $mkt_phone .
  "&mkt_email=" . $mkt_email .
  "&mkt_website=" . $mkt_website;
...
$bccapi = "mkt_question1=" . urlencode($mkt_question1) .
  "&mkt_first_name=" . urlencode($mkt_first_name) .
  "&mkt_last_name=" . urlencode($mkt_last_name) .
  "&mkt_company=" . urlencode($mkt_company) .
  "&mkt_phone=" . $mkt_phone .
  "&mkt_email=" . $mkt_email .
  "&mkt_website=" . $mkt_website .
...
curl_setopt_array($curl, [
  ...
  CURLOPT_URL => 'http://35.211.186.221/api-marketing-v2.php?' . $bccapi,
